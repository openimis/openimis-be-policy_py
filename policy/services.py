...
def update_insuree_policies(policy, audit_user_id):
    for member in policy.family.members.filter(validity_to__isnull=True):
        ...
        for insuree_policy in InsureePolicy.objects.filter(validity_to__isnull=True, insuree=member, policy=policy):
            ip, ip_created = InsureePolicy.objects.filter(id=insuree_policy.id).update_or_create(
